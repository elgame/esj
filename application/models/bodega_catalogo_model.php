<?php
class bodega_catalogo_model extends CI_Model{
	/**
	...
	public function deleteArea($idArea){
		$this->db->update('bodega_catalogo', array('status' => 'f'), "id_area = '{$idArea}'");
		return array(true, '');
	}
	...
	{
		$sql = $area==1? "AND id_padre IS NULL": '';
		$sql .= $padre? " AND id_padre = {$padre}": '';
		$query = $this->db->query("SELECT id_area, id_tipo, codigo, codigo_fin, nombre, status, id_padre, nivel
		                           FROM bodega_catalogo
		                           WHERE status = 't' {$sql}
		                           ORDER BY codigo ASC");
	...
	{
		$data = $this->db->query("SELECT id_area, id_tipo, codigo, codigo_fin, nombre, status, id_padre,
																(SELECT Count(id_area) FROM bodega_catalogo WHERE id_padre = ca.id_area) AS tiene_hijos
		                           FROM bodega_catalogo AS ca
	...
	public function getFrmAreas($id_submenu=0, $firs=true, $tipo=null, $showp=false){
		...
		if($bande==true && $firs==true && $showp==true && $res->num_rows() == 0){
			$tipo_obj = 'radio';
			$set_nombre = 'dareas';
			$txt .= '<li><label style="font-size:11px;">
			<input type="'.$tipo_obj.'" name="'.$set_nombre.'" data-uniform="false" data-tipo="0" value="0"> Padre</label>
			</li>';
			$bande = false;
		}
		$txt .= '</ul>';
    ...
    $pdf->titulo2 = 'Catalogo bodega guadalajara';
  ...
  public function getAreasXls($id_submenu=0, $firs=true, $nivel=1){
		$html = "";
		$bande = true;
		$sql_subm = $id_submenu==0? 'id_padre IS NULL': "id_padre = '{$id_submenu}'";
	    ...
	    $titulo2 = 'Catalogo bodega guadalajara';
			$html .= '<table>
      <tbody>
        <tr>
