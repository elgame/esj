  ...
  public function getEPUSPdf(){
    ...
    foreach($res as $key => $item){
      ...
      if($imprimir)
      {
        ...
        $pdf->SetMyLinks(['', '',
          base_url('panel/inventario/epus_comp_pdf?reg_product=f&'.MyString::getVarsLink(array())),
          base_url('panel/inventario/epus_comp_pdf?reg_product=t&'.MyString::getVarsLink(array()))
        ]);
        $datos = array($item->nombre_producto.' ('.$item->abreviatura.')',
          MyString::formatoNumero($item->saldo_anterior, 2, '', false),
          MyString::formatoNumero($item->entradas, 2, '', false),
