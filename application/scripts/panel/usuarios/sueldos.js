...
function addProveedor(data){
  var $this = $(this), did_proveedor = $("#did_empleado"), dproveedor = $("#dempleado"),
    tipo, salario_diario=0, factor_integracion = 0;
  if (did_proveedor.val() != '') {
    if ( $('#empleado'+data.item.id).length == 0) {
      ...
      $("#lista").append('<tr id="empleado'+data.item.id+'">'+
                        ...
                        '<td><span class="span3 pull-left">'+salario_diario+'</span> <input type="text" name="sueldo_diario[]" value="'+salario_diario+'" class="span5 pull-left vpositive" maxlength="9" '+(data.item.esta_asegurado=='f'?'readonly':'')+'></td>'+
                        '<td><span class="span3 pull-left">'+data.item.salario_diario_real+'</span> <input type="text" name="sueldo_real[]" value="'+data.item.salario_diario_real+'" class="span5 pull-left vpositive" maxlength="9"></td>'+
