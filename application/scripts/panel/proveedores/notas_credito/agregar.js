(function (closure) {
  ...
  var loadSerieFolio = function (ide) {
    var $select = $('#dserie');
    loader.create();
      $.getJSON(base_url+'panel/proveedores_notas_credito/ajax_get_series_folio/?ide='+ide, function(res){
        if(res.msg === 'ok') {
  ...
  var loadFolio = function () {
    $("#dserie").on('change', function(){
      loader.create();
      $.getJSON(base_url+'panel/proveedores_notas_credito/ajax_get_folio/?serie='+$(this).val()+'&idp='+$('#did_proveedor').val(),
      function(res){
        if(res.msg == 'ok'){
