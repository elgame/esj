...
class bodega_catalogo extends MY_Controller {
	/**
	 * Evita la validacion (enfocado cuando se usa ajax). Ver mas en privilegios_model
	 * @var unknown_type
	 */
	private $excepcion_privilegio = array(
			'bodega_catalogo/ajax_get_areas/',
			'bodega_catalogo/ajax_get_areasauto/',
			'bodega_catalogo/imprimir_catalogo/',
			'bodega_catalogo/xls_catalogo/',
		);
	...
	public function index(){
		...
		$params['areas'] = $this->bodega_catalogo_model->obtenAreas();
		...
		$this->load->view('panel/bodega_guadalajara/bodega_catalogo/listado', $params);
	...
	public function agregar(){
		...
		$this->load->view('panel/header', $params);
		$this->load->view('panel/general/menu', $params);
		$this->load->view('panel/bodega_guadalajara/bodega_catalogo/agregar', $params);
		$this->load->view('panel/footer');
	}
	...
	public function modificar(){
		...
		$this->load->view('panel/header', $params);
		$this->load->view('panel/general/menu', $params);
		$this->load->view('panel/bodega_guadalajara/bodega_catalogo/modificar', $params);
		$this->load->view('panel/footer');
	}
	...
	public function eliminar(){
		if(isset($_GET['id']{0})){
			...
			$this->load->model('bodega_catalogo_model');
			$respons = $this->bodega_catalogo_model->deleteArea($_GET['id']);

			if($respons[0])
				redirect(base_url('panel/bodega_catalogo/?msg=5'));
		}else
			$params['frm_errors'] = $this->showMsgs(1);
	}
	...
	{
		$this->load->model('bodega_catalogo_model');
		$response = $this->bodega_catalogo_model->getAreasEspesifico($_GET['id_area'], $_GET['id_padre']);
   ...
   public function ajax_get_areasauto()
   {
      ...
      echo json_encode($this->bodega_catalogo_model->ajaxAreas());
   }

  public function imprimir_catalogo()
	{
		$this->load->model('bodega_catalogo_model');
		$this->bodega_catalogo_model->listaAreas();
	}
    ...
    header("Expires: 0");
		...
		echo $this->bodega_catalogo_model->getAreasXls(0, true);
		exit;
	}
	...
	public function val_codigo($codigo)
  {
  	...
  	$datos = $this->db->query("SELECT Count(id_area) AS num from bodega_catalogo
	...
	private function showMsgs($tipo, $msg='', $title='Privilegio!'){
		switch($tipo){
			...
			break;
			case 3:
				$txt = 'El catalogo se modifico correctamente.';
				$icono = 'success';
			break;
			case 4:
				$txt = 'El catalogo se agrego correctamente.';
				$icono = 'success';
			break;
			case 5:
				$txt = 'El catalogo se elimino correctamente.';
				$icono = 'success';
			break;
		}
